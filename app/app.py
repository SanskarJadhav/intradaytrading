...
#     "Equity", 
#     ["SPY", "QQQ", "BTC-USD", "AAPL", "NVDA", "TSLA", "MSFT", "AMD", "META", "GOOGL", "AMZN", "NFLX"]
# )
...
#     # CONVICTION SCALING (Z-Score Approximation)
...
#     # Transaction costs are only incurred on trade entries/reallocations
...
#     total_net_return = (np.exp(np.sum(net_rets)) - 1) * 100

#     # --- DASHBOARD METRICS ---
#     c1, c2, c3, c4 = st.columns(4)
#     c1.metric("Information Hit Ratio", f"{hit_ratio:.2%}", help="Percentage of active trades that correctly predicted the sign of the next return.")
#     prof_color = "normal" if total_net_return >= 0 else "inverse"
#     c2.metric("Cumulative Alpha (Net)", f"{total_net_return:.2f}%", 
#               delta=f"{total_net_return:.2f}%", delta_color=prof_color)
#     c3.metric("Capacity/Participation", f"{(trades_taken/len(pred_rets)):.1%}", help="Utilization rate of the strategy based on the conviction threshold.")
#     c4.metric("N (Active Samples)", trades_taken)
...
#         if s == 0:
#             bar_colors.append('rgba(100, 100, 100, 0.1)') 
#         else:
#             color = '0, 255, 136' if s > 0 else '255, 75, 75'
#             bar_colors.append(f'rgba({color}, {c})')
...
#     fig.update_layout(title=f"Alpha Analytics: {ticker}", template="plotly_dark", 
#                       height=600, hovermode="x unified", margin=dict(t=50, b=50))
#     st.plotly_chart(fig, use_container_width=True)

#     # EQUITY CURVE
#     net_curve = (np.exp(np.cumsum(net_rets)) - 1) * 100
#     fig_equity = go.Figure()
#     fig_equity.add_trace(go.Scatter(x=times, y=net_curve, fill='tozeroy', 
